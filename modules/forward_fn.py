...
def forward_sequence_classification(model, batch_data, i2w, is_test=False, device='cpu', **kwargs):
    ...
    token_type_batch = torch.LongTensor(
        token_type_batch) if token_type_batch is not None else None
    ...
    # Forward model
    outputs = model(subword_batch, attention_mask=mask_batch,
...
def forward_word_classification(model, batch_data, i2w, is_test=False, device='cpu', **kwargs):
    # Unpack batch data
    if len(batch_data) == 4:
        (subword_batch, mask_batch,
         subword_to_word_indices_batch, label_batch) = batch_data
        token_type_batch = None
    ...
    token_type_batch = torch.LongTensor(
        token_type_batch) if token_type_batch is not None else None
    subword_to_word_indices_batch = torch.LongTensor(
    ...
    # Forward model
    outputs = model(subword_batch, subword_to_word_indices_batch,
                    attention_mask=mask_batch, token_type_ids=token_type_batch, labels=label_batch)
    loss, logits = outputs[:2]
...
def forward_sequence_multi_classification(model, batch_data, i2w, is_test=False, device='cpu', **kwargs):
    ...
    outputs = model(subword_batch, attention_mask=mask_batch,
                    token_type_ids=token_type_batch, labels=label_batch)
    # logits list<tensor(bs, num_label)> ~ list of batch prediction per class
    loss, logits = outputs[:2]
